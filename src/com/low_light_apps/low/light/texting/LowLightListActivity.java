...
public class LowLightListActivity extends ListActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Adjust_brightness = (SeekBar) findViewById(R.id.adjust_brightness);
		...
		// For emulator testing purpose
		// if (DefaultBrightnessValue == 0) {
		// DefaultBrightnessValue = 255;
		// }

		Adjust_brightness.setMax(DefaultBrightnessValue);
		restoreValue();
		setKeys();
		// setFrow();
		// setSrow();
	...
	protected void onPause() {
		...
		prefEditor.clear();
		prefEditor.putInt("opacity", Adjust_brightness.getProgress());
	...
	public void restoreValue() {
		...
		Adjust_brightness.setProgress((int) get_progress);
		...
		if (v == xParentesis) {
			...
			if (nTag.equals("num")) {
				...
				try {
					...
					start = Conversation.editMessage.getSelectionStart();
					end = Conversation.editMessage.getSelectionEnd();
					mainText = Conversation.editMessage.getText().toString();
	...
	private void setKeysOnPeriod() {

		if (mNum.getTag().equals("ABC")) {
		...
		if (Conversation.isEditMessage == true) {
			...
			if (b != null) {
				...
				try {
					...
					int start = Conversation.editMessage.getSelectionStart();
					int end = Conversation.editMessage.getSelectionEnd();
					Conversation.editMessage.getText().replace(
	...
	public String removeCharAt(String s, int pos) {
		...
		try {
			...
			// TODO: handle exception
			Utility.logger_D("Index out of bound exp" + e);
			return null;
				...
				if (cc != null && cc.length() > 0 && start > 0 && end > 0) {
					{
						...
						Conversation.editMessage.setText(removeCharAt(
								cc.toString(), start - 1));

						Conversation.editMessage.setSelection(start - 1);
	...
	protected void setConditionOnDotPress(CharSequence s) {
		...
		try {
			...
			Utility.logger_E(" getText ...-"
					+ mBShift.getBackground().equals(R.drawable.box_caps));
	...
	public void setKeyboardOnSpacePress(int start, int end, String getText) {
		...
		try {
			...
			Utility.logger_E("modify text ...@"
					+ getText.substring(start - 1, end) + "@");
			...
			if (getText.substring(start - 1, end).equals(" ")) {
				...
				if (text1.equals(". ")) {
					...
					// Utility.logger_E(" MAKE KEYBOARD CAPS ");
					setKeysOnPeriod();
