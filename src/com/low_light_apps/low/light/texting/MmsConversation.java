...
public class MmsConversation extends LowLightListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			Cursor mms_cur = getContentResolver().query(mmsUri, null,
					"thread_id = ?", selectionArgs, null);// works!!
			// Cursor mms_cur = getContentResolver().query(mmsUri, null, null,
			// null,
			// null);//all mms's
			...
			if (mms_cur != null) {
				if (mms_cur.moveToFirst()) {
					do {
						// message =
						// getMmsText(mms_cur.getString(mms_cur.getColumnIndex("_id")));
						String mmsId = mms_cur.getString(mms_cur
								.getColumnIndex("_id"));
			...
			ListAdapter adapter = new SimpleCursorAdapter(this, // Context.
					android.R.layout.simple_list_item_2, // Specify the row
															// template
															// to use (here, two
															// columns bound to
															// the
					...
					new String[] { "thread_id", "_id" }, // Array of cursor
															// columns
															// to bind to.
					new int[] { android.R.id.text1, android.R.id.text2 }); // Parallel
																			// array
																			// of
																			// which
																			// template
